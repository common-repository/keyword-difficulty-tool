...
class KDT_YahooPositionChecker {
	private $website="";
	private $keyword="";
	private $url="";
	private $start=0;
	...
	private function getRecordsAsArray($url){
		...
		preg_match_all('#class=" fz-15px(.*?)>(.*?)</span>#is', $html, $matches, PREG_SET_ORDER);
		return $matches;
	}
	...
	private function updateUrl($keyword, $start){
		...
		$new_url =  "http://search.yahoo.com/search;_ylt=A0oG7ne0xslM4DcALTRXNyoA?b=".$start."&p=$keyword";
		$this->url = $new_url;
		return $new_url;
	}
